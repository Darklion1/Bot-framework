...
class TopLevelDialog extends ComponentDialog {
    ...
    async nameStep(stepContext) {
        ...
        const promptOptions = { prompt: 'Por favor introdusca su nombre.' };
    ...
    async ageStep(stepContext) {
        ...
        const promptOptions = { prompt: 'introdusca su edad.' };
    ...
    async startSelectionStep(stepContext) {
        ...
        if (stepContext.result < 20) {
            // If they are too young, skip the review selection dialog, and pass an empty list to the next step.
            await stepContext.context.sendActivity('tienes que tener una edad mayor a 20.');
    ...
    async acknowledgementStep(stepContext) {
        ...
        await stepContext.context.sendActivity(`gracias por participar ${ userProfile.name }`);
